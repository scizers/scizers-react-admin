...
@Form.create()
class AddAdvisoryBoard extends PureComponent {
...
class AdvisoryBoardsView extends Component {
  ...
  apiRequest2 = async (params) => {
    ...
    let data = await Request.getAllContacts({ ...params, contactType: 'advisoryBoard' })
  ...
  add = async (arr) => {
    ...
    payload = { ...payload, ...arr }
    payload.contactType = ['advisoryBoard']
  ...
  render () {
    ...
    const { loading, visible, visible2, dataSource, editAdvisoryBoard, columns, disabled, showingAll } = this.state
    ...
    return (
      <PageHeaderWrapper title={Edit ? 'Edit AdvisoryBoards' : 'All AdvisoryBoards'}>
        ...
        >
          ...
          <AddAdvisoryBoard loading={loading}
                            onSubmit={(valData) => {
                              this.add(valData)
                            }}/>
